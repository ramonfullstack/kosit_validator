...
function decodeStream(stream: Buffer, filters: string[]) {
  ...
  if (filters.length === 1 && (filters[0] === "flatedecode" || filters[0] === "fl")) {
    try {
      return { data: inflateSync(stream), compressed: true, warning: null as string | null };
    } catch {
      try {
        return { data: inflateRawSync(stream), compressed: true, warning: null as string | null };
      } catch {
        return {
          data: stream,
          compressed: true,
...
export function validatePdfEmbeddedXml(input: Buffer): PdfXmlValidationResult {
